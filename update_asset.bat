...
set "properties_file=C:\Users\josue.martinez\Documents\Gol\pm.properties.txt"
set "search_key1=hostaccess.sjpm.SESSION_BTP-TK180.commlink.JSAPI.lineIATA"
set "search_key2=pm.sjpm.devices"

if defined DEBUG echo [DEBUG] Buscando archivo: %properties_file%
...
if defined DEBUG echo [DEBUG] Variables de API configuradas.
...
if "%token%"=="" (
    if defined DEBUG echo [DEBUG] SALIENDO: Fallo la autenticacion (Token vacio).
    exit /b
) else (
...
if defined DEBUG (
    ...
    :: En modo debug, ejecutamos curl sin ocultar la respuesta del servidor
    ...
    :: En modo normal, ocultamos la respuesta con >nul 2>&1
    curl -s -X "POST" "%url_base%/public-api/v2/custom-field-value-cis/multiple/" ^
      -H "accept: application/json" ^
      -H "Content-Type: application/json" ^
      -H "Authorization: Bearer %token%" ^
